...
def create_realistic_plots(sim, times, all_positions, all_velocities, energies,
    ...
    scatter = ax1.scatter(final_x, final_y, c=final_r, s=40, alpha=0.8, cmap='plasma')
    ...
    for i in range(0, N_particles, step):
        ...
        ax2.scatter(x_traj[0], y_traj[0], color=colors[i], s=30, marker='o', zorder=5)
        ax2.scatter(x_traj[-1], y_traj[-1], color=colors[i], s=30, marker='s', zorder=5)
    ...
    ax5.scatter(final_distances, theoretical_periods_h, c=colors[:N_particles], s=50, alpha=0.8)
    ...
    scatter6 = ax6.scatter(final_r, final_z, c=colors[:N_particles], s=40, alpha=0.8)
    ...
    try:
        create_ring_animation(times_hours, positions, R_saturn, ring_inner_km, ring_outer_km)
    except Exception as e:
        print(f"Could not create animation: {e}")
    
    plt.show()

def create_ring_animation(times, positions, R_saturn, ring_inner, ring_outer):
    ...
    scat = ax.scatter(positions[0, :, 0], positions[0, :, 1], c=colors, s=30, alpha=0.8)
